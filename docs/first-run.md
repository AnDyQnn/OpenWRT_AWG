...
> помнить отдельный пароль. Системный пароль `openwrt` нужен только для входа по
> **внешнему SSH-клиенту** (`ssh root@192.168.88.1`).
...
- Пароль root для внешнего SSH — команда `passwd` (по SSH или во встроенной Консоли).
