...
| Прямая ссылка | https://github.com/AnDyQnn/OpenWRT_AWG/releases/latest/download/gateway.img.gz |
| Все релизы | https://github.com/AnDyQnn/OpenWRT_AWG/releases |
...
Образ собирается в CI, вручную ничего запускать не нужно:

**Actions → release-image → Run workflow.** Версия берётся из
`linux-gw/gateway/VERSION`, тег получается вида `linux-gw-vX.Y.Z`. Альтернативно —
запушить тег `linux-gw-vX.Y.Z`, workflow сработает сам.

Что он делает: скачивает контейнеры из GHCR по тегу версии (**не пересобирает** —
так прошивка гарантированно содержит ровно то, что раздаёт канал обновлений, и не
тратится ~15 минут на компиляцию amneziawg-go), запекает их в образ, собирает
`.img`, проверяет содержимое и выпускает релиз с приложенным `gateway.img.gz`.

Занимает около получаса. Если релиз с таким тегом уже есть — файл в нём заменяется.
