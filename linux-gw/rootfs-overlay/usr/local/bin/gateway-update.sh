...
do_apply(){
    ...
    # Подчищаем осиротевшие переименованные контейнеры (<hex>_gw-*). Docker так
    # переименовывает контейнер, который не смог удалить при пересоздании — это
    # случается, если команду отдал процесс, живущий внутри этого же контейнера.
    # Тогда контейнера с нужным именем нет, health-gate его не находит и валит
    # накат. Причину убрали (панель запускает накат отвязанно), но остатки от
    # прежних версий чистим, чтобы обновление не спотыкалось о чужой мусор.
    for orph in $(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^[0-9a-f]+_gw-' ); do
        _log "APPLY: убираю осиротевший контейнер $orph"
        docker rm -f "$orph" >/dev/null 2>&1
    done
